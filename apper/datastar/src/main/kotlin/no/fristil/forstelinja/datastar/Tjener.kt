 ...
 * Datastar morfer inn hver av dem der id-en står.
 */
fun biter(vararg biter: String): String = biter.joinToString("\n")
  ...
  routing {
    ...
    post("/svar") {
      ...
      call.respondDatastar {
        patchElements(biter(topp(tilstand), brett(tilstand, tilstand.meg?.navn, kommuner, feil)))
      }
    }
    ...
    get("/hendelser") {
      ...
      call.respondDatastar {
        ...
        suspend fun tomSkjemaet() {
          patchSignals("vedtak" to "", "hjemmel" to "", "kommune" to "", "felle" to "")
        }
        ...
        suspend fun send() {
          ...
          if (forr.tavle != na.tavle) {
            ...
            patchElements(tavle(na), selector = "#tavle", mode = ElementPatchMode.INNER)
          }
        }
