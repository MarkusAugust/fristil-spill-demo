...
const APPER = [
  {
    ...
    venter: { transport: "SSE", format: "HTML", hendelse: "datastar-patch-elements" },
  },
  {
    navn: "tanstack",
...
for (const app of APPER) {
  ...
  const avlyttet = await side.evaluate(
    () => (window as unknown as { forstelinjaLedning?: { avlyttet?: boolean } }).forstelinjaLedning?.avlyttet === true,
  )
  if (!avlyttet) si("avlyttingen er ikke lastet, «/panel-avlytt.js» mangler i <head>")
  ...
  await side
    .waitForFunction(
      () => ((window as unknown as { forstelinjaLedning?: { antall: number } }).forstelinjaLedning?.antall ?? 0) > 0,
      undefined,
      { timeout: 20000 },
    )
  ...
  const nyttelast = (await panel.locator(".panel__nyttelast").textContent()) ?? ""
  if (/Ingenting har kommet/.test(nyttelast)) si("panelet sier at ingenting har kommet")
  if (/Avlyttingen er ikke lastet/.test(nyttelast)) si("panelet mangler avlyttingen")
   ...
   */
  const merkelapp = (await panel.locator(".panel__merkelapp").textContent()) ?? ""
  const lapper = await panel.locator(".panel__linjelapp").allTextContents()
  const fraStrommen = lapper.filter((l) => l.includes("SSE"))

  for (const [hva, ventet] of Object.entries(app.venter)) {
    if (!fraStrommen.some((l) => l.includes(ventet))) {
      si(`ingen strømlinje oppgir ${hva} «${ventet}». Loggen sier: ${JSON.stringify(lapper)}`)
    }
  }
  if (!/\d/.test(merkelapp)) si(`merkelappen oppgir ingen størrelse: «${merkelapp}»`)

  // Loggen skal ha sin egen merkelapp per linje, slik at man ser at det kom
  // ulike ting til ulike tider.
  if (lapper.length === 0) si("ingen av loggens linjer har en merkelapp")
  ...
  const plass = await side.evaluate(() => {
    ...
    return {
      ...
      utenforSkjermen: pr.left < 0 || pr.top < 0 || pr.right > window.innerWidth + 1,
    }
  })
  if (!plass) si("fant ikke panelet")
    ...
    if (plass.hoyde < 120) si(`nyttelastruta er bare ${plass.hoyde} piksler høy`)
  ...
  const ring = await side.evaluate(() => {
    ...
    prove.className = "panel-lyser"
    prove.textContent = "markering"
    document.body.append(prove)
    const stil = getComputedStyle(prove)
    const svar = { stil: stil.outlineStyle, bredde: stil.outlineWidth, farge: stil.outlineColor }
    prove.remove()
    return svar
  })
...
if (funn.length > 0) {
  console.error(`Fant ${funn.length} avvik i panelet:\n${funn.map((f) => `  - ${f}`).join("\n")}`)
  process.exit(1)
}
